...
app.get("/proxy", async (req, res) => {
  ...
  try {
    ...
    // Replace all href/src to route through your proxy
    html = html.replace(/(href|src)="([^"]*)"/g, (match, attr, link) => {
      if (
        ...
        // Convert relative root link to absolute
        const baseUrl = new URL(url);
        return `${attr}="/proxy?url=${encodeURIComponent(baseUrl.origin + link)}"`;
      }
      return match; // leave relative links as is (optional: handle them too)
    });
